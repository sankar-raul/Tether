...
export const varifyOtp = async (req, res) => { // varify otp for signup varification
    ...
    try {
        ...
        if (isValid) {
            ...
            const cred =  [email, username, hashedPassword, fullname]
            const [ data ] = await pool.execute("insert into users (email, username, password, fullname) value (?, ?, ?, ?)", cred)
            const userObject = {
                email,
                username,
                fullname
            }
            const [[ refresh_token, access_token ], _] = await Promise.all([RefreshToken.issue(data.insertId), updateUserInfo(data.insertId, userObject)])
            // console.log(data)
...
export const update = async (req, res) => {
    ...
    try {
        ...
        if (data[0].affectedRows == 1) {
            await updateUserInfo(id, {[what]: changeTo})
